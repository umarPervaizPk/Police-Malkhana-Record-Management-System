...
<div class="filters-container">
    <form action="print_final.php" method="GET">
        ...
        <input type="date" name="from_decision_date" id="from_decision_date" value="<?php echo isset($_GET['from_decision_date']) ? $_GET['from_decision_date'] : ''; ?>" />

        <label for="to_decision_date">تاریخ فیصلہ تک:</label>
        <input type="date" name="to_decision_date" id="to_decision_date" value="<?php echo isset($_GET['to_decision_date']) ? $_GET['to_decision_date'] : ''; ?>" />

        <label for="court_order">عدالتی حکم:</label>
        <select name="court_order" id="court_order">
            <option value="">تمام حکم</option>
            <?php while ($row = $court_orders_result->fetch_assoc()) { ?>
                <option value="<?php echo $row['court_order']; ?>" <?php echo isset($_GET['court_order']) && $_GET['court_order'] == $row['court_order'] ? 'selected' : ''; ?>>
                    <?php echo $row['court_order']; ?>
                </option>
            <?php } ?>
        </select>
        ...
        <select name="judge" id="judge">
            <option value="">تمام جج</option>
            <?php while ($row = $judges_result->fetch_assoc()) { ?>
                <option value="<?php echo $row['judge']; ?>" <?php echo isset($_GET['judge']) && $_GET['judge'] == $row['judge'] ? 'selected' : ''; ?>>
                    <?php echo $row['judge']; ?>
                </option>
            <?php } ?>
        </select>
        ...
        <select name="status" id="status">
            ...
            <?php while ($row = $statuses_result->fetch_assoc()) { ?>
                <option value="<?php echo $row['status']; ?>" <?php echo isset($_GET['status']) && $_GET['status'] == $row['status'] ? 'selected' : ''; ?>>
                    <?php echo $row['status']; ?>
                </option>
            <?php } ?>
        </select>

        <label for="decision_date_filter">تاریخ فیصلہ:</label>
        <select name="decision_date_filter" id="decision_date_filter">
            <option value="has_decision_date" <?php echo isset($_GET['decision_date_filter']) && $_GET['decision_date_filter'] == 'has_decision_date' ? 'selected' : ''; ?>>فیصلہ شدہ</option>
            </select>
...
<script>
    ...
    document.getElementById('printFilteredBtn').addEventListener('click', function () {
        ...
        printWindow.document.write('<h2> فہرست  فیصلہ شدہ مال مقدمات جمع شدہ مالخانہ سرکل ــــــــــــــــــــــــــــــ</h2>');
